...
#use rs232(baud=115200, xmit=PIN_C6, rcv=PIN_C7, stream=UART)   //Comunicación serial a 115200bps, 1 bit de paro, sin paridad
#zero_ram
...
#include <usb_desc_cdc.h> //Librería USB
      ...
      while (true)
      {
         ...
         if(usb_enumerated()) // Espera a que el dispositivo sea enumerado
         {
            if (usb_cdc_kbhit())
            {
               ...
               putc(c,UART); //Envía la información por UART
            }

            if (kbhit(UART))
            {
               u=getc(UART); //Recibe información por UART
               usb_cdc_putc(u); //Envia información hacia la computadora por puerto emulado USB
            }
         }
      }
   }
